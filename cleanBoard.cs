...
namespace BoardGameCS
{
    class Program
    {
        ...
        private static void DisplayErrorCode(int errorNumber)
        {
            ...
            if (errorNumber == 1)
            {
                Console.WriteLine("Piece entered is invalid");
            }
            else if (errorNumber == 2)
            {
                Console.WriteLine("Row or Column entered are out of range");
            }
        ...
        private static void ListPossibleMoves(string[,] board, int[,] playersPieces, string nextPlayer, MoveRecord[] listOfMoves)
        {
            ...
            for (int i = 1; i < NumberOfPieces + 1; i++)
            {
                ...
                if (isDame)
                {
                    int dameRow = newRow + direction;
                    if (ValidMove(board, dameRow, currentColumn))
                    {
                        Console.WriteLine(piece + " can move to " + dameRow + " , " + currentColumn);
                        numberOfMoves++;
                        listOfMoves[numberOfMoves].Piece = piece;
                        listOfMoves[numberOfMoves].NewRow = dameRow;
                        listOfMoves[numberOfMoves].NewColumn = currentColumn;
                        listOfMoves[numberOfMoves].CanJump = false;
                    }
                }
        ...
        private static void MakeMove(string[,] board, int[,] playersPieces, int[,] opponentsPieces, MoveRecord[] listOfMoves, int pieceIndex)
        {
            ...
            if (pieceIndex > 0)
            {
                ...
                if (jumping)
                {
                    ...
                    if (middlePiece[0] != piece[0])
                    {
                        string middlePieceIndex = "";
                        for (int i = 1; i < middlePiece.Length; i++)
                        {
                            middlePieceIndex = middlePiece[i] + middlePieceIndex;
                            opponentsPieces[Convert.ToInt32(middlePieceIndex), 0] = -1;
                            opponentsPieces[Convert.ToInt32(middlePieceIndex), 1] = -1;
                        }
        ...
        private static void Game()
        {
            ...
            while (!gameEnd)
            {
                ...
                if (nextPlayer == "a")
                {
                    Console.WriteLine("do you want to save a game?(y/n)");
                    ...
                    Ano = Ano + 1;
                    Console.WriteLine("Player A took: " + Ano + " moves");
                ...
                else
                {
                    ...
                    if (!ListEmpty(listOfMoves))
                    {
                        pieceIndex = GetRandomMove(listOfMoves);
        ...
        private static void SaveGames(int[,] a, int[,] b)
        {

            StreamWriter save = new StreamWriter("edge1.txt");
            for (int index = 0; index < NumberOfPieces + 1; index++)
            {
                save.WriteLine(a[index, Row]);
                save.WriteLine(a[index, Column]);
                save.WriteLine(a[index, Dame]);
            }
            for (int index = 0; index < NumberOfPieces + 1; index++)
            {
                save.WriteLine(b[index, Row]);
                save.WriteLine(b[index, Column]);
                save.WriteLine(b[index, Dame]);
            }
            save.Close();
            save.Close();
        }

        private static int GetRandomMove(MoveRecord[] listOfMoves)
        {
            ...
            {
                if (listOfMoves[i].Piece == "")
                {
                    NumberOfMoves = i;
                    break;
                }
            }
            return rnd.Next(1, NumberOfMoves);
